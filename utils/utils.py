...
def sample_mask(
    shape, p=0.0015, p_noise=0.05, max_seq=24, min_seq=12, rng=None
...
def get_hist_mask(observed_mask, for_pattern_mask=None, target_strategy='hybrid', rng=None):
    if rng is None:
        rand = np.random.random
        randint = np.random.randint
    else:
...
def missed_eval_np(predict, true, mask):
    ...
    # )
    evalpoints_total = np.sum((1 - mask))
    mape = np.sum(np.absolute((predict - true) * (1 - mask)) / (np.absolute(true * (1 - mask)) + 1e-5)) / evalpoints_total
...
def calc_quantile_CRPS(target, forecast, eval_points, mean_scaler, scaler):
    # target(B,L,N)  forecast(B,nsamples,L,N)  eval_points(B,L,N)
